...
public class ActionsJoueur extends JPanel {
	 ...
	 * correspondant. Je suis pas sûr que ce soit la meilleure option, un refactor
	 * est
	 * probable car on risque de très vite avoir énormément de code dans ces
	 * fichiers.
	 * En attendant, voici en gros comment on réalise la partie "Active" du
	 * controlleur,
	 * celle qui fait vraiment avancer le jeu.
	 * La méthode actionPerformed est appelé par un widget graphique, comme un
	 * bouton ou
	 * autre.
	 */
	private final class ActionFinirTour implements ActionListener {
		...
		public void actionPerformed(ActionEvent evt) {
				...
				for (int y = 0; y < monde_actuel.getDimY(); y++) {
					...
					if (case_courante.getGroupePions().getCombinaison().getPeuple() == peuple_joueur) {
						...
						if ((peuple_joueur.getType() == TypesPeuples.MAGES)
								&& (case_courante.getTypeRessource() == TypesSymboles.SOURCE_MAGIQUE)
								&& !(combinaison_joueur.getDeclin())) {
							pts_gagnes++;
						}
			...
			joueur_courant.addPoints(pts_gagnes); // on ajoute les points gagnés au nombre de points que le joueur a
													// déjà
	...
	/**
	 * La classe qui suit est semsiblement la même chose, à la différence qu'elle
	 * peut
	 * être utilisé avec n'importe quelle widget Swing, en intéragissant avec la
	 * souris.
	 * Il est ainsi possible de programmer des actions lorsque la souris survol le
	 * widget
	 * ou lorsque l'utilisateur clique dessus.
	 *
	 * Ici, la classe suppose que le widget est de type JButton mais cela peut être
	 * remplacé par n'importe quelle classe de widget Swing, y compris les custom
	 * comme
	 * CaseView...
	 */
